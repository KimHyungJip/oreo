...
class UserController{
    ...
    signup = async(req,res,next) =>{
        ...
        if(!dupCheck){
            ...
            const hashedPwd = crypto.pbkdf2Sync(password, salt, 99999, 64, 'sha512').toString('base64')
            const signUpData = await this.userService.createUser(hashedPwd,phone,email,address,salt)
    ...
    login = async(req,res,next) =>{
        ...
        const hashedPwd = crypto.pbkdf2Sync(password, userSalt, 99999, 64, 'sha512').toString('base64')
        if(user.password!==hashedPwd){
            res.status(403).send({errorMessage : '비밀번호가 틀렸습니다.'})
        }else if(user.email !== email){
            res.status(403).send({errorMessage : '이메일이 존재하지 않습니다.'})
        }else{
            const user_id = user.user_id
            const payload = {user_id}
            const accessToken = jwt.sign(payload,process.env.ACCESSTOKEN_SECRET_KEY,{expiresIn:"3600s"})
            const refreshToken = jwt.sign(payload,process.env.REFRESHTOKEN_SECRET_KEY,{expiresIn:"360000s"})
    ...
    validationCheck = async(req,res,next)=>{
        ...
        if (!token){
            res.satus(401).send({tokenNotFound:"토큰이 만료되었거나 없습니다."})
        }else{
